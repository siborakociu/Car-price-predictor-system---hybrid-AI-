...
def load_model_assets():
    ...
    try:
        ...
        return model, label_encoders, feature_cols, statistics
    except Exception as e:
        st.error(f"Error loading model files: {e}")
        st.info("Please ensure you have run the Jupyter notebook to train the model first.")
        return None, None, None, None

model, label_encoders, feature_cols, statistics = load_model_assets()
...
def prepare_input_features(input_data, label_encoders):
    ...
    for col in categorical_cols:
        if input_data[col] in label_encoders[col].classes_:
            df[f'{col}_encoded'] = label_encoders[col].transform([input_data[col]])[0]
        else:
...
with tab1:
    ...
    with col1:
        ...
        make = st.selectbox(
            "Make",
            options=sorted(makes),
            index=sorted(makes).index('Ford') if 'Ford' in makes else 0,
        ...
        model_name = st.text_input(
            "Model",
            value="F-150",
            help="Specific model of the vehicle"
        )
        
        # Get available body types
        body_types = list(label_encoders['body'].classes_)
        body = st.selectbox(
            "Body Type",
            options=sorted(body_types),
            index=sorted(body_types).index('SUV') if 'SUV' in body_types else 0,
